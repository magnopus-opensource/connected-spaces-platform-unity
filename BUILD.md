...
To build, run a standard configure/build/install CMake triplet from the root directory.
...
This should produce an `install` directory with `bin`, `lib` and `include` subdirectories (dependent on platform).

### Installing to Unity

> [!IMPORTANT]
> When building the libraries for usage in Unity on macOS, it is necessary to set the CMake variable `CMAKE_OSX_ARCHITECTURES` to `arm64`, or linking will fail.
...
### Running Tests in Unity Test Project
...
- Once the binaries, libraries and generated C# files are in place, make sure the `CspUnityTests.asmdef` file has a dependency on `ConnectedSpacesPlatform.Unity.Core.asmdef`, so that the Unity tests can build and run successfully.
