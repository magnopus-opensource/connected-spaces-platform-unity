...
/*
 * Defines in C# an awaitable member function that wraps a callback-based CSP functionality to support asynchronous scenarios.
 * The awaitable function relies on a newly defined callback, which gets rooted and is kept alive until the await completes.
 * 
 * Use this macro to conveniently make use of a generated awaitable member function and await for it:
 *
 *   // Awaitable member function fully generated by this macro:
 *   public System.Threading.Tasks.Task<extra.test.TestBooleanResult> LogAfterSecondsAsync(bool boolValue,int seconds);
 *   ...
 *   var result = await logSystem.LogAfterSecondsAsync(true, 5);
 *
 * instead of manually setting the callback and handle its lifetime for async await scenarios:
 *
 ...
 *   {
 *       // ... Something to do once the await completes ...
 *       // Manually unroot the callback since we no longer need to keep it alive
 *       ConnectedSpacesPlatformDotNet.AsyncLifetime.Unroot(callback);
 *   });
 *   
 *   // Manually root the callback, or we risk that the GC will destroy it while awaiting before we use it.
